...
When Figma mode is selected in the extension, the active email build must stay inside the generated Figma workspace folder named with the current IST date/time first, followed by the selected node name, for example `[27-08-2026] [10∶30 PM] Enphase IQ Air/`. The timestamp uses a Windows-safe visual colon (`∶`) instead of the filesystem-forbidden ASCII colon. All assets and the pre-created `TemplateName.html` file live inside that folder; edit only that pre-created HTML file. Do **not** create `generated-emails/`, `references/`, copied templates, or any extra workspace-level folders for a Figma-mode build. Do **not** edit files under `src/assets/personas/standard/references/`.
...
- Scan the copy and brief for URLs before finalizing. Extract every URL present (CTA destinations, text links, social links, view-online, legal/unsubscribe, and any bare or annotated URLs), match each to its corresponding CTA, link, image link, or social icon, and wire it as the real `href` value. Do not leave placeholder `href` values (such as `#`, `URL_HERE`, or an empty href) when the brief or copy supplies the destination. If a link target is required but no URL exists anywhere in the provided inputs, flag it to the user rather than inventing one.
